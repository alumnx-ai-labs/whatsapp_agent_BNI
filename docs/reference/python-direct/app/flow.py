"""flow.py — the conversation state machine. This is the "brain" wiring together
CRM lookup, context-aware greeting, pain-point validation, and scheduling.
"""
import re

from app import crm, context, pain_point_agent, scheduling_agent
...
async def _handle_start(session, phone: str) -> str:
    customer = crm.find_customer_by_phone(phone)

    if customer:
        session.customer = customer
        ctx = context.get_context_by_customer_id(customer["customer_id"])
        set_state(phone, State.ASK_PAIN_POINT)
        greeting = context.build_returning_greeting(customer, ctx)
...
async def _handle_name_capture(session, text: str) -> str:
    ...
    customer = crm.register_new_customer(session.phone, name, business_name)
    session.customer = customer
